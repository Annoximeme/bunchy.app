...
export function CircleChat({
  ...
  useEffect(() => {
    ...
    try {
      source = new EventSource(`/api/circles/${circleId}/stream`);
    ...
    const poll = setInterval(async () => {
      ...
      try {
        const result = await api<{ messages: ChatMessage[] }>(
          `/api/circles/${circleId}/messages?after=${encodeURIComponent(cursorRef.current)}`,
        );
  ...
  async function react(messageId: string, emoji: string) {
    ...
    try {
      await api(`/api/circles/${circleId}/messages/${messageId}`, {
        method: "PATCH",
  ...
  return (
    <div className="card-surface flex h-[min(70vh,40rem)] flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-line px-4 py-2.5">
        <h2 className="text-sm font-semibold">Circle chat</h2>
        <span
