...
export function CircleMembershipButton({
  ...
  const body = () => {
        ...
        return (
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm text-ink-soft">
              Leave this circle? You&rsquo;ll lose access to the chat.
            </span>
            <Button
              variant="danger"
              size="sm"
              ...
              onClick={() =>
                act(() =>
                  api(`/api/circles/${circleId}/membership`, { method: "DELETE" }),
                )
              }
            >
              Leave
    ...
    if (status === "INVITED") {
      return (
        <Button
          ...
          onClick={() =>
            act(() =>
              api(`/api/circles/${circleId}/membership`, {
                method: "POST",
    ...
    if (status === "REMOVED") {
      return <p className="text-sm text-muted">You can&rsquo;t rejoin this circle.</p>;
    }

    if (isFull) {
      return <p className="text-sm text-muted">This circle is full.</p>;
    }
...
export function JoinRequestList({
  ...
  async function respond(profileId: string, action: "approve" | "decline") {
    ...
    setError(null);
    try {
      await api(`/api/circles/${circleId}/members/${profileId}`, {
        method: "PATCH",
        json: { action },
      });
      router.refresh();
 ...
 */
export function CircleAssistant({ circleId }: { circleId: string }) {
  ...
  async function run(task: "summary" | "activity_idea") {
    ...
    try {
      ...
      }>(`/api/circles/${circleId}/assist`, { method: "POST", json: { task } });

      if (task === "summary") setSummary(result.summary ?? "Nothing to catch up on.");
      else if (result.suggestion) setIdea(result.suggestion);
      else setError("Not enough to go on yet — try once the circle has talked a bit.");
    } catch (cause) {
      setError(errorMessage(cause));
    } finally {
      setBusy(null);
    ...
    <Card>
      ...
      {idea && (
        <div className="mt-3 rounded-[var(--radius-control)] bg-surface-sunken p-3">
          ...
          <a
            href={`/activities/new?circleId=${circleId}&title=${encodeURIComponent(idea.title)}&description=${encodeURIComponent(idea.description)}`}
