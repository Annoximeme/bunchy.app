...
/**
 * Live circle chat over Server-Sent Events.
 *
...
export async function GET(
  ...
  try {
    ...
    await listMessages(circleId, viewer.profileId, { limit: 1 });
    ...
    const stream = new ReadableStream({
      start(controller) {
        ...
        timer = setInterval(async () => {
          ...
          try {
            const messages = await listMessages(circleId, viewer.profileId, {
              after: cursor,
              limit: 50,
            });
            ...
            // Membership revoked mid-stream, or the circle went away.
            stop();
          }
        }, POLL_INTERVAL_MS);
      },
